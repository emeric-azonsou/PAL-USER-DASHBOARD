...
export class TransactionsService {
  ...
  searchCashOutTransactions(credentials: string, userId: string, data: any) {
    let params = new HttpParams();
    ...
    // const url = `http://127.0.0.1:8000/api/getcashouttransactionsreport/${userId}`;
    const url = `${environment.getCashOutTransactionsReport}${userId}`;
  ...
  searchCashInTransactions(credentials: string, userId: string, data: any) {
    let params = new HttpParams();
    const { status, dateFrom, dateTo, country, currency, operator } = data;
      // if(from & to) {
        params = params.append("dateFrom", dateFrom);
    ...
    // const url = `http://127.0.0.1:8000/api/getcashintransactionsreport/${userId}`;
    const url = `${environment.getCashOutTransactionsReport}${userId}`;
    return this.http.get(url, { params: params, headers: headers }).pipe(
      map((transaction: any) => {
        const transactions = transaction.data.map((values) => {
